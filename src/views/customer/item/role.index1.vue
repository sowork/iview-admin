        ...
        <Modal v-model="modal1" title="角色管理" @keydown.enter.native="httpRequest.next()">
            <Form ref="formItem" :model="formItem" :rules="ruleInline" :label-width="80">
                ...
                <FormItem label="权限配置" :label-width="0">
                    <Scroll>
                        <Tree :data="data3" :load-data="loadData" show-checkbox multiple></Tree>
                    </Scroll>
                </FormItem>
            </Form>
            <div slot="footer">
...
<script>
    export default {
        ...
        methods: {
            initData () {
                this.axios.get('auth/roles').then(response => {
            ...
            * actionModal (name, method) {
            ...
            store () {
                this.axios.post('auth/store/role', this.formItem).then(response => {
                    this.modal1 = false;
                    this.initData();
                });
            },
            update () {
                ...
                this.axios.post('auth/update/role/' + this.formItem.id, this.formItem).then(response => {
                    this.modal1 = false;
                });
            },
            destroy () {
                this.formItem._method = 'delete';
                this.axios.post('auth/destroy/role/' + this.formItem.id, this.formItem).then(response => {
                    this.initData();
                });
            },
            dblClick (row, status) {
