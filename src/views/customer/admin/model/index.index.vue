    ...
    <div>
        ...
        <Modal :loading="loading" v-model="modal1" title="指标管理" @keydown.enter.native="httpRequest.next()" width="800" @on-visible-change="onVisibleChange">
            <Form ref="formItem" :model="formItem" :rules="ruleValidate" :label-width="80">
                ...
                <FormItem label="结果类型" prop="result_type">
                    <Select v-model.number="formItem.result_type">
                        <Option v-for="item in result_types" :value="item.value" :key="item.value">{{ item.name }}</Option>
                    </Select>
                </FormItem>
                ...
                <FormItem
                    ...
                    <Row>
                        ...
                        <Col span="1" style="text-align: center">-</Col>
                        <Col span="4">
                        <FormItem :prop="'items.' + index + '.level_suggest'" :rules="ruleValidate.level_suggest">
                            <Input type="text" v-model="item.level_suggest" placeholder="指标建议"></Input>
                        </FormItem>
                        </Col>
                        <Col span="2" offset="1">
                ...
                <FormItem>
                    <Row>
                        <Col span="12">
                        <Button type="dashed" long @click="handleAdd" icon="plus-round">添加指标参考级别</Button>
                        </Col>
                    </Row>
                </FormItem>
            </Form>
            ...
            return {
                ...
                editInlineColumns: [
                    ...
                    },
                    {
                        ...
                        align: 'center',
                        render: (h, params) => {
                            for (let type of Array.from(this.result_types)) {
                                if (type.value === params.row.result_type) {
                                    return h('span', type.name);
                                }
                            }
                        }
                ...
                ruleValidate: {
                    ...
                    result_type: [
                        {required: true, type: 'number', message: '结果类型不能为空', trigger: 'change'}
                    ],
                    level_suggest: [
                        {required: true, message: '建议描述不能为空', trigger: 'change'}
                    ]
                },
                formItem2: {
                    index_code: '',
        ...
        methods: {
            ...
            * actionModal (name, method, index = 0, modalName = 'modal1', isReset = false) {
            ...
            dblClick (row, index) {
                ...
                this.formItem = row;
                this.httpRequest = this.actionModal('formItem', 'update', index, 'modal1', true);
