...
<template>
    <div>
        ...
        <Modal :loading="loading" v-model="modal3" title="参考值管理" @keydown.enter.native="httpRequest3.next()" width="900">
            <Form ref="formItem3" :model="formItem3" :label-width="90">
                <FormItem
                        :key="index + 'parent'"
                    ...
                    <Row>
                        ...
                        <Col span="4" offset="1">
                            <Button type="dashed" long @click="handleAdd4(index)" icon="plus-round">添加自定义常量</Button>
                        </Col>
                        <Col span="2" offset="1">
                    ...
                    <!--<Form >-->
                        <FormItem
                            ...
                            <Row style="margin-top: 20px;">
                                <Col span="4" style="text-align: center">
                                    <span>自定义常量值{{ sindex }}</span>
                                </Col>
                                <Col span="4">
                                    <FormItem :key="sindex + 'sub'" :prop="'items.' + index + '.items.' + sindex + '.start_age'" :rules="ruleValidate3.start_age">
                                        <Input type="text" v-model.number="sitem.start_age" placeholder="开始年龄"></Input>
                                    </FormItem>
                                </Col>
                                <Col span="1" style="text-align: center">-</Col>
                                <Col span="4">
                                    <FormItem :prop="'items.' + index + '.items.' + sindex + '.end_age'" :rules="ruleValidate3.end_age">
                                        <Input type="text" v-model.number="sitem.end_age" placeholder="结束年龄"></Input>
                                    </FormItem>
                                </Col>
                                <Col span="1" style="text-align: center">-</Col>
                                <Col span="4">
                                    <FormItem :prop="'items.' + index + '.items.' + sindex + '.custom_ref_value'" :rules="ruleValidate3.custom_ref_value">
                                        <Input type="text" v-model="sitem.custom_ref_value" placeholder="自定义参考值"></Input>
                                    </FormItem>
                                </Col>
                                <Col span="2" offset="1">
                                    <Button type="ghost" @click="handleRemove4(index, sindex)">删除</Button>
                                </Col>
                            </Row>
                        </FormItem>
                    <!--</Form>-->
    ...
    export default {
        ...
        data () {
            return {
                ...
                ruleValidate3: {
                    ...
                    custom_ref_value: [
                        {required: true, message: '自定义常量值不能为空', trigger: 'blur'}
                    ]
                }
            };
        },
        ...
        methods: {
            ...
            * actionModal (name, method, index = 0, modalName = 'modal1', isReset = false) {
            ...
            handleAdd4 (index) {
                this.formItem3.items[index].items.push({
                    start_age: '',
                    end_age: '',
                    custom_ref_value: ''
                });
            ...
            handleRemove4 (index, sindex) {
                this.formItem3.items[index].items.splice(sindex, 1);
            },
            showQuotas (id) {
            ...
            showRefs (id) {
                this.axios.get('{{base_host_v1}}/item/refs/' + id, {
                    ...
                    if (response.data.data.length > 0) {
                        for (let ref of response.data.data) {
                            ref.items = ref.custom_refs;
                        }
