                ...
                >
                    <FormItem label="用户姓名：" prop="name">
                        <div style="display:inline-block;width:300px;">
                            <Input v-model="formItem.user_name" ></Input>
                        </div>
                    </FormItem>
                    <FormItem label="用户手机：" prop="user_tel" >
                        <div style="display:inline-block;width:204px;">
                            <Input v-model="formItem.user_tel" @on-keydown="hasChangePhone"></Input>
                        </div>
                    ...
                    <FormItem label="邮箱：">
                        <div style="display:inline-block;width:204px;">
                            <Input v-model="formItem.user_email"></Input>
                        </div>
                    </FormItem>
                    <FormItem label="登录密码：">
...
export default {
    ...
    methods: {
        getIdentifyCode () {
            ...
            this.$refs['formItem'].validate((valid) => {
                if (valid) {
                    ...
                    this.axios.post('{{host}}/code', {
                        tel: this.formItem.user_tel
                    }).then(response => {
        ...
        saveEdit () {
            this.$refs['formItem'].validate((valid) => {
                if (valid) {
                    if (this.phoneHasChanged && this.formItem.user_tel !== this.initPhone) { // 手机号码修改过了而且修改之后的手机号和原来的不一样
        ...
        saveEditPass () {
            this.$refs['editPasswordForm'].validate((valid) => {
                if (valid) {
                    ...
                    // you can write ajax request here
                    this.axios.post('{{host_v1}}/user/resetpwd', this.editPasswordForm).then(response => {
        ...
        init () {
            this.axios.get('{{host_v1}}/user/detail').then(response => {
                this.formItem = response.data.data;
                this.initPhone = response.data.data.user_tel;
                this.school_name = response.data.data.school.school_name;
            });
        },
        cancelInputCodeBox () {
            this.inputCodeVisible = false;
            this.formItem.user_tel = this.initPhone;
        },
        submitCode () {
            let vm = this;
            ...
            if (this.securityCode.length === 0) {
                ...
                this.axios.post('{{host_api}}/sms/verify/code', {
