...
<template>
    <div>
        ...
        <Modal :loading="loading" v-model="modal1" title="套餐管理" @keydown.enter.native="httpRequest.next()">
            <Form ref="formItem" :model="formItem" :rules="ruleValidate" :label-width="80">
                <FormItem label="套餐名称" prop="combo_name">
                ...
                <FormItem label="有效天数" prop="valid_day">
                    <Input v-model.number="formItem.valid_day" placeholder="不填表示永久有效"></Input>
                </FormItem>
                ...
                <FormItem label="套餐描述" prop="combo_desc">
        ...
        <Modal v-model="modal3" title="内容管理" @keydown.enter.native="httpRequest3.next()" width="1000">
            <Form ref="formItem3" :model="formItem3" :rules="ruleValidate3" label-position="top">
                <div v-for="(item, index) in formItem3.items">
                    <Row>
                        <!--<Col span="4">-->
                            <!--<FormItem label="产品" :prop="'items.' + index + '.product_id'" :rules="ruleValidate3.product_id">-->
                                <!--<Select @on-change="changeProduct" v-model="item.product_id">-->
                                    <!--<Option v-for="item in products" :value="item.item_id" :key="item.item_id">{{ item.item_name }}</Option>-->
                                <!--</Select>-->
                            <!--</FormItem>-->
        ...
        data () {
            ...
            return {
                ...
                ruleValidate: {
                    ...
                    combo_type: [
                        {required: true, type: 'number', message: '套餐类别不能为空', trigger: 'change'}
                    ],
                    start_date: [
...
//                    ],
        ...
        methods: {
//            changeProduct (val) {
//                this.axios.get('{{base_host_v1}}/item/sub/item', {
//                    params: {
//                        id: val
//                    }
//                }).then(response => {
...
//            },
            initData () {
                Promise.all([
                    ...
                    this.axios.get('{{base_host_v1}}/orgs/items', {
                        params: {
                            item_type: 2
                        }
                    }),
                ...
                ]).then(([orgs, courses, products, rooms, users]) => {
